export const consultas_diagnos = {
  ...
  getDiagnosID:
    "SELECT diag.`id_diag` , cie.`nombre_enfermedad`,cie.`codigoCIE`, diag.`tipo_diag`, diag.`desc_diag` FROM `diagnostico_tbl` as diag INNER JOIN `enfermedades_cie.10` as cie ON diag.`codigoCIE` = cie.`codigoCIE` WHERE diag.`id_diag` = ?",
